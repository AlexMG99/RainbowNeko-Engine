	...
	if (object)
	{
		...
		if (comp_camera)
		{
			if (ImGui::CollapsingHeader("Camera"))
			{
				...
				if (ImGui::InputFloat3("Position", (float*)&comp_camera->c_frustum.pos, 2))
				{
					comp_camera->UpdateFrustum(true);
				}
				ImGui::Separator();

				if (ImGui::InputFloat("Near Plane", &comp_camera->c_frustum.nearPlaneDistance, 5) ||
					ImGui::InputFloat("Far Plane", &comp_camera->c_frustum.farPlaneDistance, 5) ||
					ImGui::InputFloat("FOV", &comp_camera->c_frustum.verticalFov, 0.1))
				{
